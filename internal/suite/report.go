...
func Print(w io.Writer, report Report) {
	...
	fmt.Fprintln(w, "participant         pass fail required noise structure markdown order metadata security md_bytes md_words noise_hits dup_lines elapsed_ms")
	for _, metric := range report.Metrics {
		fmt.Fprintf(w, "%-19s %4d %4d %7d/%-3d %3d/%-3d %5d/%-3d %5d/%-3d %3d/%-3d %5d/%-3d %8d %8d %8d %10d %9d %10d\n",
...
func WriteMarkdown(path string, report Report) error {
	...
	for _, result := range report.Cases {
		...
		fmt.Fprintf(&b, "| %s | %s | %s | %d/%d | %d/%d | %d/%d | %d/%d | %d/%d | %d | %d | %d | %d | %d | %d | %d | %s | `%s` |\n", result.CaseID, result.Participant, result.Status, passCount(result.Required), len(result.Required), passCount(result.Forbidden), len(result.Forbidden), passCount(result.Structure), len(result.Structure), passCount(result.Markdown), len(result.Markdown), passCount(result.Order), len(result.Order), result.Native.ContentBytes, result.Stats.Words, result.Stats.Headings, result.Stats.Links, result.Stats.CodeFences, result.Stats.NoiseHits, result.Stats.DuplicateLines, result.Native.SiteConfig, digest)
	}
	if report.Failed > 0 {
		b.WriteString("\n## Failures\n\n")
		for _, result := range report.Cases {
